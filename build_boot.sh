...
#   ./build_boot.sh --incremental   # keep out/, rebuild only what changed
...
#   BUILD_SH        Path to build.sh                     (default: alongside this script)
...
# build.sh is the sibling of this script, not part of the kernel tree.
BUILD_SH="${BUILD_SH:-$(cd "$(dirname "$0")" && pwd)/build.sh}"
...
# PARSE ARGUMENTS
# ============================================================
# "shift" cannot advance a "for arg in $@" loop, so --boot needs a while loop
# to reach its value.
while [ $# -gt 0 ]; do
    case "$1" in
        clean)
        ...
        --boot)
            if [ -z "$2" ]; then
                echo "ERROR: --boot needs a path"
                exit 1
            fi
            BOOT_IMG="$2"
            shift
            ;;
        --incremental|--no-cache)
            BUILD_ARGS+=("$1")
            ;;
        --help|-h)
            echo "Usage: $0 [--boot <img>] [--incremental] [--no-cache] [clean]"
            echo "  --boot <img>   Reuse a local boot.img instead of downloading"
            echo "  --incremental  Keep out/ and rebuild only what changed"
            echo "  --no-cache     Disable ccache for this build"
            echo "  clean          Remove work dir and output boot image"
            echo "Env: DEVICE, MAGISK_VERSION, BOOT_IMG, USE_CCACHE, KERNEL_DIR, BUILD_SH"
            exit 0
            ;;
        *)
...
if [ -n "${BOOT_IMG}" ]; then
    ...
    if [ ! "${BOOT_IMG}" -ef "${WORK_DIR}/boot.img" ]; then
...
chmod +x "${BUILD_SH}"
KERNEL_DIR="${KERNEL_DIR}" "${BUILD_SH}" "${BUILD_ARGS[@]}"
